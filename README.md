...
Также будем замерять время работы только 2 пункта с помощью rdtsc - инструкция ассемблера, предоставляемая процессором Intel x86 для чтения значения тактовых счетчиков процессора.
...
<img src = "./pictures/prof3.png" width="500" height="250">

#### Оптимизация хэш-функции
Профайлер показал, что хеш-функция crc32 работает дольше всех среди других функций. Будем оптимизировать эту функцию.
...
В языке программирования существует функция CRC32 с использованием интринсиков, которая используется для вычисления контрольной суммы (хеша) для заданных данных. Оптимизируем функцию, используя интринсик функцию CRC32.

Для использования интринсиков, таких как _mm_crc32_u8, данные должны быть выровнены в соответствии с требованиями процессора. В противном случае могут возникнуть проблемы производительности или даже ошибки выполнения. Поэтому при считывании каждого слова из файла необходимо выравнивать слово по границе 16 байт, поскольку у меня самое длинное слово составляет 16 байт.
...
Таблица с разультатами:
| Оптимизация              |      Количество тактов        | Абсолютное ускорение  |
|:------------------------:|:-----------------------------:|:---------------------:|
| Без оптимизации          |  $(4.17 \pm 0.15) \cdot 10^9$ |          1            |
| CRC32                    |  $(3.18 \pm 0.37) \cdot 10^9$ |          1.31         |
...
| CRC32                    |  $(3.18 \pm 0.37) \cdot 10^9$ |          1.31         |              1.31         |
| strcmp                   |  $(2.12 \pm 0.35) \cdot 10^9$ |          2.01         |              1.5          |

#### Оптимизация HT_Find

<img src = "./pictures/prof2.png" width="500" height="250">

Теперь профайлер показал, что функция HT_Find работает дольше других функций. Будем оптимизировать эту функцию.
...
Посмотрим в godbolt как компилируется функция:
...
| Без оптимизации          |  $(4.17 \pm 0.15) \cdot 10^9$ |          1            |              1            |
| CRC32                    |  $(3.18 \pm 0.37) \cdot 10^9$ |          1.31         |              1.31         |
| strcmp                   |  $(2.12 \pm 0.35) \cdot 10^9$ |          2.01         |              1.50         |
| HT_Find                  |  $(1.53 \pm 0.73) \cdot 10^9$ |          2.73         |              1.38         |

#### Увеличение размера хэш-таблицы

Оптимизировать программу можно засчет понижения нагрузки на функцию HT_Find, в которой присутствует функция strcmp. Можно уменьшить количество вызовов strcmp засчет уменьшения load factor. Если увеличить размер таблицы до заселенности(1-2 элемента на ячейку), то количество вызовов можно сократить. Поэтому хорошим load factor считается значение 1.5.

Увеличим таблицу с 5003 до 40001, тем самым уменьшив load factor с 12 до 1.5.
...
| strcmp                   |  $(2.12 \pm 0.35) \cdot 10^9$ |          2.01         |              1.50         |
| HT_Find                  |  $(1.53 \pm 0.73) \cdot 10^9$ |          2.73         |              1.38         |
| Размер таблицы           |  $(1.02 \pm 0.58) \cdot 10^9$ |          4.08         |              2.67         |
...
$$K = \frac{\tau_{\text{before}}}{\tau_{\text{after}}} \times \frac{1}{N_{str}} \times 1000$$
