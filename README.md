...
Подход заключается в том, чтобы загрузить оба строки в регистры xmm0 и xmm1, затем сравнить их с помощью инструкции pcmpeqb, которая устанавливает биты в регистре результата в соответствии с сравнением каждого байта. Затем мы используем инструкцию pmovmskb, чтобы перенести маску сравнения в регистр общего назначения, чтобы проверить, были ли обнаружены несовпадения.
