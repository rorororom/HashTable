...
Посмотрим на графическое сравнение изменений в функции ROL при разных уровнях оптимизации:
...
###### rol(unsigned long):
- В данной версии функции нет сохранения и восстановления регистров, что снижает накладные расходы.
...
- Оптимизированный код остаётся лаконичным и читабельным.
...
$$ \text{Дисперсия} = \frac{\sum_{i=1}^{n} (x_i - \bar{x})^2}{n} $$

Где:
- \( n \) - количество входных данных (размер выборки).
- \( x_i \) - значение хэш-кода для \( i \)-го входного элемента.
- \( \bar{x} \) - среднее значение хэш-кода по всем входным данным.
