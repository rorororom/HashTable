...
void ListElemInit(Node* elem, char* value, Node* prev, Node* next);
...
void PopElement(List* list, Node* position)
{
    ...
    ListElemInit(position, 0, nullptr, nullptr);
...
int VerifyMeaningData(struct List* list)
{
    ...
    if (size == list->length - 1) return 0;
    else return 1;
...
void CreateNode(FILE* dotFile, Node* id, const char* value, Node* next, Node* prev) {
    fprintf(dotFile, "%p"
...
void CreateNewGraph()
{
    ...
    char command[1000];
    sprintf(command, "dot -Tpng /Users/aleksandr/Desktop/HASH_TABLE/grapth.dot -o /Users/aleksandr/Desktop/HASH_TABLE/grapth/%s", filename);
    system(command);
