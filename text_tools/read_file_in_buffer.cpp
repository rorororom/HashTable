...
void fill_text_and_count_word(struct BufferForFile *source)
{
    ...
    source->words = 1; // Начальное значение считается словом
    for (int i = 0; i < source->size; i++) {
        if (source->buffer[i] == '\n' || source->buffer[i] == ' ') {
            source->words++;
        }
    ...
    // Выделяем память для хранения указателей на слова
    ...
    // Заполняем массив указателями на начало каждого слова
